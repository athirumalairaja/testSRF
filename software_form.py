...
def return_project_id():
    ...
    response = requests.get(f'{url}?name={config.software_project_name}', auth=(config.username, config.password), verify=False)

    logging.debug(f"{response.request.method} - {response.status_code} - {response.url}")
    if response.status_code != requests.codes.ok:
        raise ObjectNotFound(f"{response.status_code} - Failed to search project: name={config.software_project_name}")

    search_response = response.json()
    if not search_response['count']:
        raise ObjectNotFound(f"{response.status_code} - Project doesn't exist: name={config.software_project_name}")
    else:
        project_id = search_response['results'][0]['id']

    logging.info(f'{response.status_code} - Found project: name={config.software_project_name}, id={project_id}')
    return project_id
...
def get_ee_id():
    ...
    response = requests.get(f'{url}?name={config.software_eename}', auth=(config.username, config.password), verify=False)
    logging.debug(f"{response.request.method} - {response.status_code} - {response.url}")

    if response.status_code != 200:
        error_message = response.json()
        logging.error(f'{response.status_code} - Error fetching EE id: name={config.software_eename}, msg={error_message}')
        return None

    results = response.json().get('results', [])
    if not results:
        logging.error(f'{response.status_code} - EE not found: name {config.software_eename}, msg: {response.json()}')
        return None

    ee_id = results[0]["id"]
    logging.info(f'{response.status_code} - Found Execution Environment: name={config.software_eename}, id={ee_id}')
    return ee_id
...
def software_main(software_vars):
    # Set up logging
    logging_setup()
    ...
    try:
        ...
        organization_id = get_organization_id(config.software_organizations)
        if not organization_id:
            raise ObjectNotFound(f"Organization not found: name={config.software_organizations}")

        project_id = return_project_id()  # Getting project ID
        # Create inventory with timestamp
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        inventory_name = f"software-inventory-{timestamp}"
        ...
        # Use software_vars instead of vars
        hostnames = software_vars["hostnames"]
        # Add hosts to the inventory
        for hostname in hostnames:
            add_host_to_inventory(inventory_id, hostname)
         
        temp_credentials = []
        playbook = config.software_playbook

        credentials = config.software_credentials  # Getting credentials ID
        if credentials:
        ...
        # Extract email from the software_vars dictionary
        email = software_vars["email"]
        # Convert email to a list for the receivers parameter
        receivers = [email]

        # Validate job_slice_count
        job_slice_count = software_vars.get("job_slice_count", 1)  # Default to 1 if not provided
        ...
        name = f"Software-Install-{software_vars['apps_to_install']}"
        payload = {
            'name': name,
            ...
            'verbosity': 2,
            'extra_vars': json.dumps(software_vars),
            'job_slice_count': job_slice_count,  # Include job_slice_count if supported
            'job_tags': '',
            'skip_tags': '',
            'become_enabled': '',
