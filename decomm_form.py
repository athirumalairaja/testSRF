...
def return_project_id():
    ...
    response = requests.get(f'{url}?name={config.decomm_project_name}', auth=(config.username, config.password), verify=False)

    logging.debug(f"{response.request.method} - {response.status_code} - {response.url}")
    if response.status_code != requests.codes.ok:
        raise ObjectNotFound(f"{response.status_code} - Failed to search project: name={config.decomm_project_name}")

    search_response = response.json()
    if not search_response['count']:
        raise ObjectNotFound(f"{response.status_code} - Project doesn't exist: name={config.decomm_project_name}")
    else:
        project_id = search_response['results'][0]['id']

    logging.info(f'{response.status_code} - Found project: name={config.decomm_project_name}, id={project_id}')
    return project_id

# Search inventory and return its id
def return_inventory_id():
    url = config.api_url + 'inventories/'
    response = requests.get(f'{url}?name={config.decomm_inventory_name}', auth=(config.username, config.password), verify=False)

    logging.debug(f"{response.request.method} - {response.status_code} - {response.url}")
    if response.status_code != requests.codes.ok:
        raise ObjectNotFound(f"{response.status_code} - Failed to search inventory: name={config.decomm_inventory_name}")

    search_response = response.json()
    if not search_response['count']:
        raise ObjectNotFound(f"{response.status_code} - Inventory doesn't exist: name={config.decomm_inventory_name}")
    else:
        inventory_id = search_response['results'][0]['id']

    logging.info(f'{response.status_code} - Found inventory: name={config.decomm_inventory_name}, id={inventory_id}')
    return inventory_id
...
def get_ee_id():
    ...
    response = requests.get(f'{url}?name={config.decomm_eename}', auth=(config.username, config.password), verify=False)
    logging.debug(f"{response.request.method} - {response.status_code} - {response.url}")

    if response.status_code != 200:
        error_message = response.json()
        logging.error(f'{response.status_code} - Error fetching EE id: name={config.decomm_eename}, msg={error_message}')
        return None

    results = response.json().get('results', [])
    if not results:
        logging.error(f'{response.status_code} - EE not found: name {config.decomm_eename}, msg: {response.json()}')
        return None

    ee_id = results[0]["id"]
    logging.info(f'{response.status_code} - Found Execution Environment: name={config.decomm_eename}, id={ee_id}')
    return ee_id
...
def main(vars):
    ...
    try:
        ...
        organization_id = get_organization_id(config.decomm_organizations)
        if not organization_id:
            raise ObjectNotFound(f"Organization not found: name={config.decomm_organizations}")

        project_id = return_project_id()  # Getting project ID
        inventory_id = return_inventory_id()  # Getting inventory ID
        temp_credentials = []
        playbook = config.decomm_playbook

        credentials = config.decomm_credentials  # Getting credentials ID
        if credentials:
        ...
        receivers = [email]
        name = f"Decommission-{vars['vm_name']}"
        payload = {
            'name': name,
